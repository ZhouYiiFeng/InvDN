...
class InvSHBlock(nn.Module):
    def __init__(self, subnet_constructor, current_channel, channel_out, cal_jacobian=False,mode="h2r"):
    ...
    def forward(self, inps):
        ...
        if not rev:
            ...
            if self.mode == "h2r":
                ...
                x = torch.cat([packs, haarfs], dim=1)
            x = self.subInvBlk2(x, rev)
            if self.mode == "h2r":
                haarfs, packs = x.chunk(2, dim=1)
            else:
                packs, haarfs = x.chunk(2, dim=1)
...
class InvNet(nn.Module):
    def __init__(self, channel_in=3, channel_out=3,
        ...
        self.modelist=["h2r", "r2h"] * (down_num * block_num[0] // 2)
        print("Mode List : ")
        print(self.modelist)
        mc = 0
        for i in range(down_num):
            ...
            for j in range(block_num[i]):
                b = InvSHBlock(subnet_constructor, current_channel, channel_out,mode=self.modelist[mc])
                operations.append(b)
                mc += 1
